...
<div class = "box">
    ...
    <form action="PhysicsPUREMapper.php" method="post" enctype="multipart/form-data">
        <table>
            ...
            <tr class ="inputtypeindent"><td class ="inputtypeindent">Include acceptance date? ***WILL FAIL***<input type="checkbox" name="acc" id="acc"></td></tr>
    ...
    foreach ($xml->children() as $object)
    {
        ...
        if($matchedcount > 0 and $date !== '') {
            ...
            if ($accdate == '') {
                ...
                //if accepted date checkbox is set, write the Acc Date (optional due to failures)
                if ($acc == true) {
                    $writer->startElement('v1:publicationStatus');
                    $writer->writeElement('v1:statusType', 'accepted');
                    $writer->startElement('v1:date');
