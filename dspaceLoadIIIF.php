...
<div class= "central">
    ...
    <div>
        <form method="post" action="dspaceLoadIIIF.php">
    ...
    if (isset($_POST['sendDetails']))
    {
        ...
        foreach($xml->children() as $object)
        {
            ...
            if ($collection == 'stcecilia' or $collection == 'test')
            {
                $avusecount = 0;
                ...
                foreach ($avsortarray as $sortedav)
                {
                    ...
                    {
                        $urlstartpos = strpos($item, '; ht');
                        $urlstartpos = $urlstartpos + 2;
                        $urlendpos = strpos($item, '.jpg');
                        $urlendpos = ($urlendpos +4) - $urlstartpos;
                        $url = substr($item,$urlstartpos,$urlendpos);
                        $outline = '<dcvalue element = "identifier" qualifier = "imageUri">'.$url."</dcvalue>\n";
                        fwrite($file_handle_dc_out,$outline);
                        echo 'Processed Image: '.$url."<br>";
                        $processed_images++;
                    }
